...
def Crawling():
    ...
    for idx in range(1, 11):
        ...
        # titles
        titles.append(driver.find_element_by_xpath("//ul[@class='lst_detail_t1']/li[" + str(idx) + "]/dl[@class='lst_dsc']/dt[@class='tit']/a"))

        # scores
        scores.append(driver.find_element_by_xpath("//ul[@class='lst_detail_t1']/li[" + str(idx) + "]//span[@class='num']").text)
        ...
        length  = len(summary)
        times.append(summary[length - 2].text) # running time
        dates.append(summary[length - 1].text) # opening date
        ...
        print(dates[idx-1][-14:-3]) # first opening date ( not re-opening date )
